...
DEFINE_ACTION_FUNCTION(AInventory, A_Raise)
{
	...
	{
		APowerRespawnInvulnerable *invulnerability = static_cast<APowerRespawnInvulnerable *>(player->mo->FindInventory (RUNTIME_CLASS(APowerRespawnInvulnerable)));

		if ((invulnerability) && (player->ReadyWeapon) && ((player->ReadyWeapon->WeaponFlags & WIF_ALLOW_WITH_RESPAWN_INVUL) == false))
		{
