...
APlayerPawn *P_SpawnPlayer (FPlayerStart *mthing, int playernum, int flags)
{
	...
	if (( NETWORK_InClientMode() == false ) &&
		...
		( p->bSpectating == false ))
	{
		APowerup *invulnerability = static_cast<APowerup *>(p->mo->GiveInventoryType (RUNTIME_CLASS(APowerRespawnInvulnerable)));

		if (NETWORK_GetState() == NETSTATE_SERVER)
			SERVERCOMMANDS_GivePowerup (playernum, invulnerability);
	}
